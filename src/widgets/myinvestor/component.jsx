...
export default function Component({ service }) {
  ...
  const getColorClass = (value) => (value >= 0 ? "text-green-500" : "text-red-500");

  const twoDecimals = (value) => Number(Number.parseFloat(value).toFixed(2)).toLocaleString("es-ES");

  return createPortal(
    <div className="w-full mt-2">
      <Container service={service}>
        <div className="flex flex-col w-full p-2">
          {inversionesDtoList.map((inversion) => (
            <div key={inversion.id} className="flex flex-row justify-between text-xs py-1">
              <div className="flex-1 text-left">
                <span>
                  {twoDecimals(inversion.importeInicial)}
                  {inversion.divisaValorLiquidativo}
                </span>
              </div>
              <div className="flex-1 text-left">
                <span className={getColorClass(inversion.beneficio)}>
                  {twoDecimals(inversion.beneficio)}
                  {inversion.divisaValorLiquidativo} ({twoDecimals(inversion.rentabilidadTotal)}%)
                </span>
              </div>
            </div>
          ))}
